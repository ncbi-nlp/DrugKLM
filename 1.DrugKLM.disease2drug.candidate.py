...
def main(disease_name, disease_id, payload, output_json):
    ...
    # Step 1.1: Disease → Mechanism → Drugs mapping
    # === Disease → Mechanism ===
    print(disease + " - Step 1. Mechanism Discovery...")
    prompt_mechanism = "prompts/1.GPT4Mechanism.CandidateGeneration.txt"
    mechanism_prompt = load_prompt_template(prompt_mechanism, disease, payload)
    try:
        mech_response = ask_gpt([
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": mechanism_prompt}
        ], params)
    except Exception as e:
        mech_response = f"[ERROR] {e}"

    mech_response = truncate_incomplete_json_array(mech_response)

    mechanisms = []
    try:
        mechanisms = json.loads(mech_response)
    except Exception:
        print(f"[x] Failed to parse mechanisms for {disease}")
        mechanisms = []
        
    
    # === Mechanism → Drugs mapping ===
    print(disease + " - Step 2. Drugs per Mechanism...")
    prompt_mech2drug = "prompts/1.GPT4Mechanism2Drug.CandidateGeneration.txt"
    json_reasoning_response = []

    for mech in mechanisms:
        print (mech)
        # Extract mechanism fields with safe fallbacks
        mech_name = (mech.get("Mechanism") or "").strip()
        mech_role = (mech.get("Role_in_Disease") or "").strip()

        if not mech_name:
            # Skip if no mechanism name
            continue

        # Load base prompt and replace both placeholders
        mech2drug_prompt = load_prompt_template(prompt_mech2drug, disease, payload)

        # Replace [Mechanism] and [Role_in_Disease] placeholders in the prompt
        # Note: Keep comments in English as requested.
        mech2drug_prompt = mech2drug_prompt.replace("[Mechanism]", mech_name)
        mech2drug_prompt = mech2drug_prompt.replace("[Role_in_Disease]", mech_role if mech_role else "N/A")
        
        try:
            resp = ask_gpt([
                {"role": "system", "content": "You are a helpful assistant."},
                {"role": "user", "content": mech2drug_prompt}
            ], params)
        except Exception as e:
            resp = f"[ERROR] {e}"

        resp = truncate_incomplete_json_array(resp)
        try:
            drugs = json.loads(resp)
            json_reasoning_response.extend(drugs)
        except Exception:
            print(f"[!] Failed to parse drug list for mechanism: {mech_name}")
            continue

    # === Post-processing: add IDs, dedup ===
    for item in json_reasoning_response:
        drug_name = item.get("Drug", "")
        matches = find_exact_match(drug_name, target_type="Chemical")
        item["ID"] = matches[0] if matches else "[NOT_FOUND]"

    merged = {}
    for it in json_reasoning_response:
        id_str = (it.get("ID") or "").strip()
        nm_norm = _strong_norm_name(it.get("Drug", ""))

        key = ("ID", id_str) if id_str and id_str != "[NOT_FOUND]" else ("NAME", nm_norm)

        if key not in merged:
            merged[key] = dict(it)  # copy
        else:
            base = merged[key]
            ...
            print(base["Mechanism_of_Action"])
    # Re-materialize list
    json_reasoning_response = list(merged.values())
    ...
    existing_names = {_norm_name(it.get("Drug") or "") for it in json_reasoning_response if it.get("Drug")}
    ...
    json_reasoning_response, existing_ids, existing_names = _merge_topk(
        json_reasoning_response, topk_prime, "HAKE_PrimeKG",
        id_to_official, existing_ids, existing_names, _norm_name
    )
    json_reasoning_response, existing_ids, existing_names = _merge_topk(
        json_reasoning_response, topk_primewe, "HAKE_PrimeKGwE",
        id_to_official, existing_ids, existing_names, _norm_name
    )
...
"""
time python 1.KGxLM.disease2drug.candidate.py \
  --json_input input/mCRC.json \
  --output output/disease2drug/mcrc.disease2drug.candidate.jsonl
"""